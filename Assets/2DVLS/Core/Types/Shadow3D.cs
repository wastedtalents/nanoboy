...
public class Shadow3D : Light2D
{
    ...
    protected override void Initialize()
    {
        ...
        lightDimensionSupport = LightDimensionSupport._3D;
    ...
    {
        _3DObjList = Physics.OverlapSphere(transform.position, lightRadius, shadowLayer);//(transform.position + new Vector3(-lightRadius, lightRadius, 0), transform.position + new Vector3(lightRadius, -lightRadius, 0), shadowLayer);
    }
    ...
    protected override void Draw()
    {
        ...
        if (coneAngle > 0)
        {
            ...
            for (int i = 0; i < rays + 1; i++)
            {
                ...
                {
                    if (Physics.Raycast(transform.position, transform.TransformDirection(Quaternion.Euler(0, 0, coneStart) * circleRef[i]), out rhit, lightRadius, shadowLayer))
                    {
                        if (Application.isPlaying && useEvents && !unidentifiedObjects.Contains(rhit.transform.gameObject))
                            unidentifiedObjects.Add(rhit.transform.gameObject);

                        float penetration = ((rhit.distance) <= 0) ? 0 : lightPenetration;
                        Vector3 fPos = (rhit.point + (Vector3)circleRef[i] * penetration);
                        
                        verts.Add(transform.InverseTransformPoint(fPos));
                        verts.Add(Quaternion.Euler(0, 0, coneStart) * ((Vector3)circleRef[i] * lightRadius));
    ...
    protected override void UpdateTriangles()
    {
        ...
        //for (int v = 0; v < _mesh.vertexCount - 2; v += 2)
        //{
        //    tris.Add(v + 2);
        //    tris.Add(v + 1);
        //    tris.Add(v);
    ...
    public static Shadow3D Create(Vector3 _position, Color _color, float _radius = 1, float _coneAngle = 360, float _coneStart = 0, Material _material = null)
    {
        GameObject obj = new GameObject("Shadow Emitter");
        obj.transform.position = _position;

        Shadow3D l = obj.AddComponent<Shadow3D>();
